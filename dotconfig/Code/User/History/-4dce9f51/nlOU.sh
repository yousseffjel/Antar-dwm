...
if [[ $inst =~ ^[Yy]$ ]]; then
   app_pkgs="base-devel libx11 libxft libxinerama freetype2 xorg polkit-kde-agent ly kitty thunar feh picom unzip wget vim tmux lxappearance betterlockscreen visual-studio-code-bin network-manager-applet gvfs jq tlp auto-cpufreq"
   app_pkgs2="neofetch flameshot rofi dunst ffmpeg bat neovim viewnior gparted mpv playerctl brightnessctl pamixer pavucontrol ffmpegthumbnailer tumbler thunar-archive-plugin htop xdg-user-dirs pacman-contrib ttf-joypixels ttf-font-awesome noto-fonts-emoji"
   app_pkgs3="timeshift grub-btrfs brave-bin bitwarden telegram-desktop figlet opendoas rhythmbox qbittorrent trash-cli freedownloadmanager firefox-developer-edition zsync tar sudo sed grep curl nodejs npm cargo tree lazygit binutils coreutils fuse python-pip"
