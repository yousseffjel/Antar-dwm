...
if [[ $inst =~ ^[Yy]$ ]]; then
   app_pkgs="base-devel libx11 libxft libxinerama freetype2 syncthing deemix-git sxhkd local-by-flywheel xorg polkit-kde-agent ly thunar feh picom unzip unrar wget vim tmux lxappearance betterlockscreen visual-studio-code-bin network-manager-applet gvfs jq tlp auto-cpufreq"
   app_pkgs2="neofetch flameshot dunst ffmpeg xclip bat neovim viewnior gparted mpv playerctl brightnessctl pamixer pavucontrol ffmpegthumbnailer tumbler thunar-archive-plugin htop xdg-user-dirs pacman-contrib ttf-joypixels ttf-font-awesome noto-fonts-emoji deemix-gui-appimage syncthing-bin altus-bin"
   app_pkgs3="timeshift grub-btrfs brave-bin telegram-desktop figlet opendoas rhythmbox dust thorium-browser-bin qbittorrent trash-cli freedownloadmanager firefox-developer-edition zsync tar sudo xsel sed grep curl nodejs npm cargo tree lazygit binutils coreutils fuse python-pip xkblayout-state-git brightness "
