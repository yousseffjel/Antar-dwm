...
if [[ $inst =~ ^[Yy]$ ]]; then
   app_pkgs="base-devel libx11 libxft libxinerama freetype2 syncthing deemix-git sxhkd local-by-flywheel xorg polkit-kde-agent ly thunar feh picom unzip wget vim tmux lxappearance betterlockscreen visual-studio-code-bin network-manager-applet gvfs jq tlp auto-cpufreq"
